...
def estimate_transform(left_list, right_list, fix_scale = False):
    ...
    for l in left_list:
        l_prime.append((float(l[0]) - lcx, float(l[1]) - lcy))
    for r in right_list:
        r_prime.append((float(r[0]) - rcx, float(r[1]) - rcy))
...
def correct_pose(pose, trafo):
    la, c, s, tx, ty = trafo
    x, y, theta = pose
    new_p = apply_transform(trafo, pose[0:2])
    new_theta = theta + atan2(s, c)
    # --->>> This is what you'll have to implement.
    return new_p[0], new_p[1], new_theta
