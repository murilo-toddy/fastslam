    ...
    def background_draw(self):
        for l in self.landmarks:
            if l[0] == 'C':
                ...
                x = float(x)
                y = float(y)
                ll = to_world_canvas((x - float(l[3]), y - float(l[3])), self.canvas_extents, self.world_extents)
                ur = to_world_canvas((x + float(l[3]), y + float(l[3])), self.canvas_extents, self.world_extents)
...
class Points(DrawableObject):
    ...
    def draw(self, at_step):
        if self.cursor_objects:
            for cursor in self.cursor_objects:
